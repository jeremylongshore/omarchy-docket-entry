...
const helper = path.join(__dirname, "..", "bin", "docket-secure-state")
const token = `ghp_${"A".repeat(36)}`

function setup() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "docket-state-"))
  return { root, dir: path.join(root, "state", "omarchy", "docket"), env: { ...process.env, HOME: root, XDG_STATE_HOME: path.join(root, "state") } }
}
...
test("credentials publish as a private valid final JSON object", () => {
  ...
  assert.deepEqual(JSON.parse(fs.readFileSync(final, "utf8")), { token, login: "jeremylongshore", host: "github.com" })
  cleanup(x)
})

test("a symlinked final entry is replaced without writing through to its victim", () => {
...
test("same-UID final and temporary entry swaps never write through to a victim", async () => {
  ...
  const racer = spawn(process.execPath, [path.join(__dirname, "fixtures", "credential-swap-racer.js"), x.dir, victim, ready], { stdio: "ignore" })
  await waitFor(ready)
  const attempts = []
...
test("same-UID parent directory swaps cannot redirect credential publication", async () => {
  const x = setup(); fs.mkdirSync(x.dir, { recursive: true })
  const victim = path.join(x.root, "victim-parent"); fs.mkdirSync(victim)
  const ready = path.join(x.root, "parent-ready")
  const racer = spawn(process.execPath, [path.join(__dirname, "fixtures", "credential-parent-racer.js"), x.dir, victim, ready], { stdio: "ignore" })
  await waitFor(ready)
...
test("oversized or malformed credential input fails closed", () => {
  const x = setup()
  for (const input of ["x".repeat(2048), "bad\nuser\n", `${token}\nnot valid!\n`])
